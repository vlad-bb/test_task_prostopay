...
def test_should_create_hashtable(val=None):
    assert HashTable(capacity=100) is not val
...
def test_should_set_value(hash_table):
    hash_table["foo"] = "bar"
    assert hash_table["foo"] == "bar"

    hash_table.put("foo", "baz")
    assert hash_table["foo"] == "baz"
